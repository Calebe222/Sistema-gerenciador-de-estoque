...
class Database:
    def __init__(self):
        # detecta nome da coluna de quantidade no banco (qtde ou quantidade)
        self.qtde_col = 'qtde'
        try:
            conn = self.connect()
            if conn:
                with conn:
                    with conn.cursor() as cur:
                        cur.execute("SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name='produtos'")
                        cols = [r[0] for r in cur.fetchall()]
                if 'qtde' in cols:
                    self.qtde_col = 'qtde'
                elif 'quantidade' in cols:
                    self.qtde_col = 'quantidade'
                else:
                    self.qtde_col = None
                conn.close()
        except Exception:
    ...
    def get_baixo_estoque_count(self, threshold=5):
        ...
        try:
            with conn:
                with conn.cursor() as cur:
                    qt_col = self.qtde_col or 'qtde'
                    cur.execute(f"SELECT COUNT(*) FROM Produtos WHERE {qt_col} < %s", (threshold,))
        ...
        try:
            with conn:
                with conn.cursor() as cur:
                    qt_col = self.qtde_col or 'qtde'
                    cur.execute(
                        f"INSERT INTO Produtos (nome, {qt_col}, preco) VALUES (%s, %s, %s)",
                        (nome, qtde, preco)
                    )
        except Exception as e:
        ...
        try:
            with conn:
                with conn.cursor() as cur:
                    qt_col = self.qtde_col or 'qtde'
                    cur.execute(
                        f"UPDATE Produtos SET nome = %s, {qt_col} = %s, preco = %s WHERE id = %s",
        ...
        try:
            with conn:
                with conn.cursor() as cur:
                    qt_col = self.qtde_col or 'qtde'
                    cur.execute(
                        f"UPDATE Produtos SET {qt_col} = {qt_col} - %s WHERE id = %s",
                        (qtde_vendida, id_produto)
                    )
        except Exception as e:
    ...
    def insert_venda(self, nomeVenda, qtde, precoTotal, data):
        ...
        try:
            with conn:
                with conn.cursor() as cur:
                    # tenta inserir usando nomes de colunas esperados; se falhar, levanta exceção para log
                    cur.execute(
...
def telaInicial():
    ...
    # Tenta caminho original, senão procura na pasta do projeto (onde main.py está)
    bg_path = "Projetos/Sistema de Estoque/wallpaper.png"
    if not os.path.exists(bg_path):
        bg_path = os.path.join(os.path.dirname(__file__), "wallpaper.png")
    try:
        ...
        # Não há imagem de fundo; prosseguir sem ela
        pass
    ...
    logo_path = "Projetos/Sistema de Estoque/logo.png"
